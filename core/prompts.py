...
def build_reading_prompt(
    ...
    if spread_type == 3 and len(cards) == 3:
        lines.append(
            'Формат JSON: {"intro": "вступительная фраза ко всему раскладу", '
            '"short_answer": "общий вывод по всем трём картам, 2-3 предложения", '
            '"card_meaning": ['
            '"[Прошлое] Название_карты: значение, привязанное к вопросу", '
            '"[Настоящее] Название_карты: значение, привязанное к вопросу", '
            '"[Будущее] Название_карты: значение, привязанное к вопросу"'
            "], "
            '"advice": "совет по всей ситуации"}'
        )
    else:
        lines.append(
