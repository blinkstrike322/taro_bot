...
def build_reading_prompt(
    ...
    if mode == "three":
        ...
        lines.append(
            ...
            "• НЕ дублируй одно и то же в short_answer и card_meaning — они про разное: "
            "short_answer это связный сюжет всей истории с синтезом, "
            "а card_meaning это детальный разбор по позициям\n"
    ...
    if mode == "three":
        lines.append(
            ...
            '  "card_meaning": "три абзаца через пустую строку, по одному на каждую позицию. '
            'В каждом абзаце: (1) трактовка карты в её позиции, (2) её связь с соседней картой, '
            '(3) что это значит для вопроса пользователя. Связным текстом, абзацами, не списком",\n'
        ...
        lines.append(
            ...
            '    "На что смотреть: слепая зона — что сегодня легко не заметить или понять '
