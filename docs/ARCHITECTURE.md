...
### `POST /api/spread`
- Принимает: `tg_id`, `spread_type` (1, 3 или "daily"), `question` (опционально), `character_id`
- Возвращает: `reading_id`, массив карт с интерпретацией
- Логика: `draw_cards(count)` -> `interpret_reading()` -> сохранение в БД
- daily — расклад дня из 3 карт (Энергия дня / Вызов дня / Совет дня)

### `POST /api/followup`
- Принимает: `init_data`, `reading_id`, `question` (до 500 символов)
- Возвращает: `{answer, remaining}` — ответ проводницы по картам расклада
- Логика: загрузка расклада + истории вопросов -> `interpret_followup()` -> сохранение
- Лимит: 15 доп-вопросов на один расклад
...
### Пайплайн (хеджированный, см. core/llm.py)

Порядок моделей OpenCode Zen — по бенчмарку `scripts/benchmark_models.py`
(2026-08, реальные промпты раскладов):

1. `nemotron-3-ultra-free` — ~15 c, качество 99/100
2. `muse-spark-1.2-contributor-free` — ~24 c, лучший текст
3. `laguna-s-2.1-free` — ~22 c, быстрые доп-вопросы
4. `mimo-v2.5-free`, `deepseek-v4-flash-free`, `nemotron-3.5-lightning-free`
5. Хвост OpenRouter (последний рубеж)

Механика: первая модель получает 12 c форы, затем параллельно стартуют
следующие (до 3 одновременно); побеждает первый валидный ответ, остальные
попытки отменяются. Таймаут попытки 45 c. Модель, упавшая с
429/5xx/«unavailable»/таймаутом, на 5 минут выпадает из очереди (кулдаун).
Если ответ невалиден (пустой short_answer) — один дополнительный заход.
Замер латентности: `scripts/check_pipeline_latency.py` (медиана ~15-20 c).
...
## 9. Проводницы (персонажи)

Три проводницы определены в `data/characters.json` (ID сохранены для БД):

| ID | Имя | Стиль |
|----|-----|-------|
| `shadow_walker` | Селена, Дочь Луны | Поэтичная, тёплая; образы луны, воды, снов |
| `ruin_keeper` | Веста, Хранительница Очага | Прямая, веская, тёплая как очаг; конкретика |
| `spark_of_chaos` | Лилит, Искра | Дерзкая плутовка с юмором и глубиной |

### Продвинутая система характера (анти-повторение)

Каждая проводница имеет:
- `persona` — характер и биография для LLM
- `focus` — на что смотрит в первую очередь
- `moods` — 4 настроения (ротация случайно при каждом раскладе меняет тон)
- `openers` — 5 способов начать ответ (ротация)
- `forbidden` — персональные запрещённые штампы + глобальный список
- `greetings` — несколько приветствий для бота
- `system_prompt` — краткая legacy-версия

`core/prompts.py::get_system_prompt()` собирает всё это в один промпт и
добавляет фрагменты недавних ответов (`avoid_texts` из БД,
`get_recent_guide_texts`) с указанием не повторять формулировки.

### Контекстные расклады

`detect_question_category()` определяет тему вопроса (love / career /
money / decision / self / health / general) по ключевым словам, и в промпт
добавляется профильная инструкция «Фокус вопроса» — толкование привязано
к теме, а не к пересказу значений карт.
...
Доп-вопросы пользователя по уже сделанному раскладу («Спроси ещё»).
